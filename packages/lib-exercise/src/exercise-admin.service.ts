...
export class ExerciseAdminService {
   ...
   */
  static async executeAiPlan(options: AiPlanOptions): Promise<ExerciseAiExecutionResult> {
    ...
    if (
      ...
      return {
        summary:
          'Unable to create exercises: database metadata (muscles, body parts) not found. Please run: pnpm db:seed',
        createResult: null,
        updateResult: [],
        deleteResult: [],
        approvalResult: [],
    ...
    const validCreateRecords = plan.create.filter((exercise: CreateExerciseInput) => {
      const hasTranslations =
        exercise.translations &&
        Array.isArray(exercise.translations) &&
        exercise.translations.length > 0;
      const hasMuscles =
        exercise.muscles && Array.isArray(exercise.muscles) && exercise.muscles.length > 0;
      const hasBodyParts =
        exercise.bodyPartIds &&
    ...
    const updateResult: ExerciseAiExecutionResult['updateResult'] = [];
    for (const entry of plan.update) {
      try {
        const exercise =
          slugCache.get(entry.slug) ??
          (
    ...
    const deleteResult: ExerciseAiExecutionResult['deleteResult'] = [];
    if (plan.delete.length > 0) {
      const slugs = plan.delete.map((item: { slug: string }) => item.slug);
      const existing = await prisma.exercises.findMany({
        where: { slug: { in: slugs } },
        select: { id: true, slug: true },
      });

      const existingMap = new Map(existing.map((item: any) => [item.slug, item.id]));
      const idsToDelete = existing.map((item: any) => item.id);
    ...
    const approvalResult: ExerciseAiExecutionResult['approvalResult'] = [];
  ...
  static async generateExercisesWithAgent(options: {
    ...
    if (existingNames.length === 0) {
      ...
      existingNames.push(
        ...allExercises.map(
          (e: { exercise_translations: Array<{ name?: string }>; slug: string }) => {
...
CRITICAL: Use ONLY the IDs listed above. Do NOT invent IDs or use names.`;
    ...
    const metadataStore = {
      createdExerciseTypes: {} as Record<string, string>, // name -> id
      createdEquipments: {} as Record<string, string>, // name -> id
      createdMuscles: {} as Record<string, string>, // name -> id
      createdBodyParts: {} as Record<string, string>, // name -> id
    };
    ...
    const allExercises = await processBatchesInParallel<number, CreateExerciseInput>({
      items: batchIndices,
      batchSize: 1, // Process one batch index at a time, but in parallel groups
      parallelGroups: 2,
      processor: async (batch: number[]) => {
        ...
        try {
          ...
          const exercises = (result.output?.exercises || []) as CreateExerciseInput[];
      ...
      onGroupComplete: (results: CreateExerciseInput[]) => {
    ...
    const importRecords: ExerciseImportPayload[] = allExercises
      .map(
        (
          exercise: CreateExerciseInput & { approvalStatus?: ExerciseApprovalStatus }
        ): ExerciseImportPayload | null => {
          ...
          const uniqueTranslations = exercise.translations.filter((t: any) => {
          ...
          const record: ExerciseImportPayload = {
            slug: exercise.slug,
            ...
            muscles: exercise.muscles.map((m: any) => ({ id: m.id, role: m.role })), // Already validated above
          ...
          if (validatedCount % 5 === 0) {
            onProgress?.(
              ...
              `Validati ${validatedCount}/${allExercises.length} esercizi...`
            );
          }
      ...
      .filter((record): record is ExerciseImportPayload => {
    ...
    if (importRecords.length === 0) {
      throw new Error(
        'No valid exercises generated (missing required fields: translations, muscles, or bodyPartIds)'
      );
    }
    ...
    const importResult = await this.import(importRecords, {
      ...
      sharedContext: {
        metadata: {
          createdExerciseTypes: sharedContext.metadata.createdExerciseTypes as Record<
            string,
            string
          >,
        },
   ...
   */
  private static async generateAiPlan(prompt: string): Promise<ExerciseAiPlan> {
    ...
    // Build ID maps for AI (ID → English name for reasoning)
    const muscleIdMap = metadata.muscles.map((m: any) => `${m.id}: ${m.name}`).join(', ');
    const bodyPartIdMap = metadata.bodyParts.map((bp: any) => `${bp.id}: ${bp.name}`).join(', ');
    const equipmentIdMap = metadata.equipment.map((e: any) => `${e.id}: ${e.name}`).join(', ');
...
- Do NOT create exercises if you don't have valid muscle IDs and body part IDs to use.`;
    ...
    try {
      ...
      const resultWithObject = result as unknown as { object?: unknown | Promise<unknown> };
      let structuredObject: unknown | null = null;
      if (resultWithObject.object) {
        if (resultWithObject.object instanceof Promise) {
          structuredObject = await resultWithObject.object;
        } else if (
          typeof resultWithObject.object === 'object' &&
          resultWithObject.object !== null &&
          'then' in resultWithObject.object &&
          typeof (resultWithObject.object as { then?: unknown }).then === 'function'
        ) {
          structuredObject = await (resultWithObject.object as Promise<unknown>);
        } else {
