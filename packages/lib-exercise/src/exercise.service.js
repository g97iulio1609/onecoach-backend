...
export class ExerciseService {
    ...
    static async getById(id, locale = DEFAULT_LOCALE, options = {}) {
    ...
    static async getBySlug(slug, locale = DEFAULT_LOCALE, options = {}) {
        const normalizedLocale = this.normalizeLocale(locale);
        const includeTranslations = options.includeTranslations ?? false;
        const exercise = await prisma.exercises.findUnique({
            where: {
    ...
    static async update(id, payload, options = {}) {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (updateData.translations) {
                ...
                await Promise.all(uniqueTranslations.map((translation) => tx.exercise_translations.upsert({
                    where: {
                        exerciseId_locale: {
                            exerciseId: id,
            ...
            const diff = compare(previousSnapshot, snapshot);
        ...
        this.invalidateCaches();
        return this.mapExerciseToLocalized(result, normalizedLocale, options.includeTranslations ?? false);
    }
    static async setApprovalStatus(id, status, options) {
        const result = await prisma.$transaction(async (tx) => {
            const existing = await tx.exercises.findUnique({
                where: { id },
                include: EXERCISE_INCLUDE,
            ...
            const snapshot = this.buildSnapshot(updated);
            await this.recordVersion(tx, id, updated.version, previousSnapshot, snapshot, options.userId, {
                event: 'APPROVAL',
                approvalStatus: status,
            });
            return updated;
    ...
    static buildExerciseCacheKey(id, locale, includeTranslations) {
        return `exercise:${id}:${locale}:${includeTranslations ? 'all' : 'single'}`;
    }
    static buildListCacheKey(options) {
    ...
    static pickTranslation(translations, locale) {
        ...
        const sameLanguage = translations.find((translation) => translation.locale.startsWith(language));
        if (sameLanguage) {
    ...
    static mapExerciseToLocalized(exercise, locale, includeTranslations) {
    ...
    static mapListRowToLocalized(exercise, locale) {
        const translations = exercise.exercise_translations
            .map((translation) => ({
            locale: translation.locale.toLowerCase(),
            name: translation.name,
    ...
    static async recordVersion(tx, exerciseId, version, previousSnapshot, snapshot, userId, metadata) {
        const diff = compare(previousSnapshot ?? {}, snapshot);
    ...
    static async countSearchFullText(term, options) {
    ...
    static getSearchConditions(term, locale, filters) {
        const query = term.trim();
        const preparedTerm = query.replace(/[:!&|']/g, ' ');
        const conditions = [
        ...
        if (filters.approvalStatus) {
            conditions.push(Prisma.sql `e."approvalStatus" = ${filters.approvalStatus}::"ExerciseApprovalStatus"`);
        }
        if (filters.exerciseTypeId) {
            conditions.push(Prisma.sql `e."exerciseTypeId" = ${filters.exerciseTypeId}`);
        }
        if (filters.muscleIds?.length) {
            const arraySql = Prisma.sql `ARRAY[${Prisma.join(filters.muscleIds.map((muscleId) => Prisma.sql `${muscleId}`))}]::text[]`;
            conditions.push(Prisma.sql `EXISTS (SELECT 1 FROM "exercise_muscles" em WHERE em."exerciseId" = e.id AND em."muscleId" = ANY(${arraySql}))`);
        }
        if (filters.bodyPartIds?.length) {
            const arraySql = Prisma.sql `ARRAY[${Prisma.join(filters.bodyPartIds.map((partId) => Prisma.sql `${partId}`))}]::text[]`;
            conditions.push(Prisma.sql `EXISTS (SELECT 1 FROM "exercise_body_parts" eb WHERE eb."exerciseId" = e.id AND eb."bodyPartId" = ANY(${arraySql}))`);
        }
        if (filters.equipmentIds?.length) {
            const arraySql = Prisma.sql `ARRAY[${Prisma.join(filters.equipmentIds.map((equipmentId) => Prisma.sql `${equipmentId}`))}]::text[]`;
            conditions.push(Prisma.sql `EXISTS (SELECT 1 FROM "exercise_equipments" ee WHERE ee."exerciseId" = e.id AND ee."equipmentId" = ANY(${arraySql}))`);
        }
        return Prisma.join(conditions, ' AND ');
    }
    static prepareCreateData(payload, options) {
        // Normalize and deduplicate translations by locale (keep first occurrence)
        const normalizedTranslations = payload.translations.map(this.normalizeTranslationInput.bind(this));
        const seenLocales = new Set();
        ...
        const englishTranslation = translations.find((translation) => translation.locale === DEFAULT_LOCALE);
    ...
    static prepareUpdateData(payload, existing, userId) {
        ...
        if (payload.approvalStatus && payload.approvalStatus !== existing.approvalStatus) {
            ...
            if (payload.approvalStatus === ExerciseApprovalStatus.APPROVED && userId) {
                exerciseUpdate.approvedById = userId;
            }
            else {
                exerciseUpdate.approvedById = null;
            }
        }
    ...
    static normalizeTranslationInput(translation) {
        return {
            locale: translation.locale.toLowerCase(),
    ...
    static prepareRelatedRelations(exerciseId, relations) {
        if (!relations.length) {
            return [];
        }
