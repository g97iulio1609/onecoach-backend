 ...
 */
import { calculateWeightFromIntensity, calculateIntensityFromWeight, } from './helpers/intensity-calculator';
 ...
 */
export function calculateSetWeights(set, oneRepMaxKg, weightIncrement = 2.5) {
    let newWeight = set.weight ?? null;
    let newWeightLbs = set.weightLbs ?? null;
    let newIntensityPercent = set.intensityPercent ?? null;
 ...
 */
export async function calculateWeightsInProgram(userId, program) {
    // Raccogli tutti i catalogExerciseId presenti nel programma
    const exerciseIds = new Set();
    program.weeks.forEach((week) => week.days.forEach((day) => day.exercises.forEach((exercise) => {
    ...
    const weightIncrement = userProfile.weightIncrement !== null && userProfile.weightIncrement !== undefined
        ? typeof userProfile.weightIncrement === 'object' && 'toNumber' in userProfile.weightIncrement
            ? userProfile.weightIncrement.toNumber()
        ...
        weeks: program.weeks.map((week) => ({
            ...
            days: week.days.map((day) => ({
                ...
                exercises: day.exercises.map((exercise) => {
                    ...
                    const updatedSetGroups = exercise.setGroups?.map((group) => ({
                        ...group,
                        baseSet: calculateSetWeights(group.baseSet, oneRepMaxKg, weightIncrement),
                        sets: group.sets.map((set) => calculateSetWeights(set, oneRepMaxKg, weightIncrement)),
                    }));
                    return {
                        ...exercise,
 ...
 */
export async function updateProgramWeightsForExerciseId(userId, catalogExerciseId) {
    try {
        ...
        for (const program of programs) {
            ...
            const needsUpdate = normalizedProgram.weeks.some((week) => week.days.some((day) => day.exercises.some((exercise) => exercise.catalogExerciseId === catalogExerciseId)));
            if (!needsUpdate) {
                continue;
            }
