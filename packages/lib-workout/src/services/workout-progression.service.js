...
export class WorkoutProgressionService {
     ...
     */
    static syncOccurrenceWithOneRepMax(occ, oneRepMax) {
        // Deep copy per immutabilità
        const newOcc = deepClone(occ);
        if (!oneRepMax || oneRepMax <= 0)
            return newOcc;
        newOcc.exercise.setGroups.forEach((group) => {
            ...
            const syncSet = (s) => {
                if (s.intensityPercent && s.intensityPercent > 0) {
                    ...
                    if (s.weightMax) {
                        s.intensityPercentMax = Number(calculateIntensityFromWeight(s.weightMax, oneRepMax).toFixed(1));
                    }
                }
            };
     ...
     */
    static previewProgression(occurrences, params, selectedIndices, oneRepMax) {
        ...
        updatedOccurrences.forEach((occ, idx) => {
            ...
            occ.exercise.setGroups.forEach((group) => {
        ...
        if (oneRepMax) {
            updatedOccurrences = updatedOccurrences.map((occ) => this.syncOccurrenceWithOneRepMax(occ, oneRepMax));
        }
        return updatedOccurrences;
    }
    static updateSet(set, type, value, oneRepMax) {
        switch (type) {
            case 'linear_weight': {
                ...
                if (oneRepMax && oneRepMax > 0) {
                    ...
                    if (set.weightMax) {
                        set.intensityPercentMax = Number(calculateIntensityFromWeight(set.weightMax, oneRepMax).toFixed(1));
                    }
                }
                break;
            }
