 ...
 */
const matchCache = new SimpleCache({
    max: 1000,
    ttl: 1000 * 60 * 5,
});
/**
 ...
 */
const ngramIndexCache = new SimpleCache({
    max: 5,
    ttl: 1000 * 60 * 30,
});
// ==================== N-GRAM INDEX ====================
 ...
 */
function generateNgrams(str, n = 3) {
 ...
 */
function buildNgramIndex(exercises) {
    const index = new Map();
    ...
    for (let i = 1; i <= b.length; i++) {
        ...
        for (let j = 1; j <= a.length; j++) {
            if (b.charAt(i - 1) === a.charAt(j - 1)) {
                ...
                currRow[j] = Math.min(prevRow[j - 1] + 1, // substitution
...
export class ExerciseMatcherService {
    ...
    static BATCH_CONCURRENCY = 10;
    /**
     * Carica tutti gli esercizi dal database per il matching
     */
    static async loadExercises(locale = 'en') {
     ...
     */
    static async getNgramIndex(locale) {
     ...
     */
    static async findCandidatesWithNgram(normalizedInput, locale, maxCandidates = 50) {
     ...
     */
    static async matchExercise(name, locale = 'en', threshold = 0.7) {
        ...
        if (scores.length < 3) {
            for (const exercise of exercises) {
                for (const translation of exercise.translations) {
                    ...
                    if (scores.some((s) => s.exercise.id === exercise.id && s.translation.name === translation.name)) {
                        continue;
                    }
                    const nameSimilarity = calculateSimilarity(normalizedInput, normalizeName(translation.name));
                    let searchTermsSimilarity = 0;
                    if (translation.searchTerms.length > 0) {
                        const maxSearchTermSimilarity = Math.max(...translation.searchTerms.map((term) => calculateSimilarity(normalizedInput, normalizeName(term))));
     ...
     */
    static async matchExercises(exercises, locale = 'en', threshold = 0.7) {
     ...
     */
    static applyMatches(exercises, matches) {
     ...
     */
    static async createMissingExercise(name, _sourceFile, userId, locale = 'en') {
