 ...
 */
function parseSetsNotation(notation) {
    const result = {};
 ...
 */
function parseWeight(value, sourceUnit = 'kg') {
...
export class FileParserService {
     ...
     */
    static async parseSpreadsheet(file, options) {
        const warnings = [];
        try {
            ...
            const sheetsToProcess = file.sheetIndex !== undefined
            ...
            for (let sheetIdx = 0; sheetIdx < sheetsToProcess.length; sheetIdx++) {
                ...
                for (let rowIdx = 1; rowIdx < data.length; rowIdx++) {
                    ...
                    if (!exerciseName || String(exerciseName).trim() === '') {
                        // Riga vuota potrebbe indicare cambio giorno
                    ...
                    if (columnMap.has('sets')) {
                        ...
                        if (setsValue) {
                            ...
                            if (parsed.sets) {
                                ...
                                if (parsed.intensityPercent)
                                    exercise.intensityPercent = parsed.intensityPercent;
                            }
                            else {
                    ...
                    if (columnMap.has('weight')) {
                        ...
                        if (weightValue) {
                            const parsed = parseWeight(weightValue, options.sourceWeightUnit);
                            exercise.weight =
                                parsed.weight ||
                    ...
                    if (columnMap.has('rest')) {
                        ...
                        if (restValue) {
                            exercise.rest = parseRest(restValue);
                        }
                    }
                    // RPE
            ...
            const programName = file.name
                ...
                .replace(/[_-]/g, ' ')
                .trim();
            const program = ImportedWorkoutProgramSchema.parse({
     ...
     */
    static async parseDocument(file, _options, aiContext) {
        const warnings = [];
        if (!aiContext) {
            return {
                success: false,
...
Rispondi SOLO con un JSON valido nel formato ImportedWorkoutProgram.`;
     ...
     */
    static async parseImage(file, _options, aiContext) {
        const warnings = [];
        if (!aiContext) {
            return {
                success: false,
...
Rispondi SOLO con un JSON valido nel formato ImportedWorkoutProgram.`;
     ...
     */
    static async parsePDF(file, _options, aiContext) {
        const warnings = [];
        if (!aiContext) {
            return {
                success: false,
...
IMPORTANTE: Rispondi SOLO con un JSON valido nel formato ImportedWorkoutProgram, senza markdown o testo aggiuntivo.`;
     ...
     */
    static async parseFile(file, options, aiContext) {
     ...
     */
    static async parseFiles(files, options, aiContext) {
        const programs = [];
        const errors = [];
        const warnings = [];
        ...
        for (const chunk of chunks) {
            const results = await Promise.all(chunk.map((file) => this.parseFile(file, options, aiContext)));
     ...
     */
    static combinePrograms(programs) {
