 ...
 */
export function calculateWeightFromIntensity(oneRepMax, intensityPercent) {
 ...
 */
export function calculateIntensityFromWeight(weight, oneRepMax) {
 ...
 */
export function calculateWeightFromRPE(oneRepMax, reps, rpe) {
    if (oneRepMax <= 0) {
    ...
    if (intensity === undefined) {
        ...
        const interpolated = lowerIntensity + (upperIntensity - lowerIntensity) * ((rpe - lowerRpe) / 0.5);
 ...
 * @returns Intensità percentuale (0-100)
 */
export function calculateIntensityFromRPE(reps, rpe) {
    if (reps < 1)
        reps = 1;
    if (reps > 12)
        reps = 12;
 ...
 */
export function calculateRPEFromWeight(weight, oneRepMax, reps) {
    if (oneRepMax <= 0 || weight <= 0)
        return 8; // Fallback
 ...
 * @returns RPE stimato (6.5-10)
 */
export function calculateRPEFromIntensity(intensityPercent, reps) {
    if (reps < 1)
        reps = 1;
    if (reps > 12)
        reps = 12;
 ...
 */
export function estimateOneRMFromReps(reps, weight, rpe = 10) {
 ...
 */
export function syncSetValues(focusField, values, oneRepMax, reps, weightIncrement) {
    const result = {};
    ...
    switch (focusField) {
        case 'weight':
            ...
            if (values.weight && values.weight > 0) {
                result.intensityPercent = roundTo(calculateIntensityFromWeight(values.weight, oneRepMax), 1);
            ...
            if (values.weightMax && values.weightMax > 0) {
                result.intensityPercentMax = roundTo(calculateIntensityFromWeight(values.weightMax, oneRepMax), 1);
                if (reps) {
        ...
        case 'intensity':
            ...
            if (values.intensityPercent && values.intensityPercent > 0) {
                result.weight = roundToPlateIncrement(calculateWeightFromIntensity(oneRepMax, values.intensityPercent), increment);
            ...
            if (values.intensityPercentMax && values.intensityPercentMax > 0) {
                result.weightMax = roundToPlateIncrement(calculateWeightFromIntensity(oneRepMax, values.intensityPercentMax), increment);
                if (reps) {
        ...
        case 'rpe':
            ...
            if (values.rpe && values.rpe >= 6.5 && reps) {
                ...
                result.weight = roundToPlateIncrement(calculateWeightFromIntensity(oneRepMax, intensity), increment);
                result.weightLbs = roundTo(kgToLbs(result.weight), 1);
            }
            if (values.rpeMax && values.rpeMax >= 6.5 && reps) {
                ...
                result.weightMax = roundToPlateIncrement(calculateWeightFromIntensity(oneRepMax, intensityMax), increment);
            }
            break;
    }
    return result;
