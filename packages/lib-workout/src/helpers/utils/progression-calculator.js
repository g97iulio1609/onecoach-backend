 ...
 */
export function applyProgression(baseSet, progression, setNumber // 1-based
) {
    const result = { ...baseSet };
    // Trova lo step di progressione che si applica a questa serie
    const applicableStep = progression.steps.find((step) => setNumber >= step.fromSet && setNumber <= step.toSet);
    if (!applicableStep) {
    ...
    switch (progression.type) {
        ...
        case 'percentage': {
            ...
            if (result.intensityPercent !== null && result.intensityPercent !== undefined) {
                result.intensityPercent = Math.min(100, Math.max(0, (result.intensityPercent ?? 0) + totalAdjustment));
            }
            break;
        }
        case 'rpe': {
 ...
 */
export function generateSetsFromGroup(group) {
    const sets = [];
...
export function calculateGroupSummary(group) {
    ...
    // Peso/Intensity
    const hasWeight = sets.some((s) => s.weight !== null && s.weight !== undefined);
    const hasIntensity = sets.some((s) => s.intensityPercent !== null && s.intensityPercent !== undefined);
    if (hasIntensity) {
        const intensities = sets
            .map((s) => s.intensityPercent)
 ...
 */
export function isUniformGroup(group) {
