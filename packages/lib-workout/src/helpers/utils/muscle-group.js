 ...
 */
export function getMuscleGroupFromName(name) {
    const normalized = name.trim().toLowerCase();
    // Controlla se è già un valore valido
    if (ALLOWED_MUSCLE_GROUPS.has(normalized)) {
        return normalized;
    }
    ...
    const aliasKey = Object.keys(MUSCLE_GROUP_ALIASES).find((key) => normalized.includes(key));
    if (aliasKey) {
