 ...
 */
export function normalizeMuscleGroups(value) {
    const rawGroups = ensureArrayOfStrings(value).map((group) => group.toLowerCase());
 ...
 */
export function normalizeExerciseSets(value) {
    ...
    const sets = value
        ...
        const reps = raw.reps !== undefined
            ? ensureNumber(raw.reps)
            : raw.repRange
                ? ensureNumber(raw.repRange)
                : undefined;
        const duration = raw.duration !== undefined ? ensureNumber(raw.duration) : ensureNumber(raw.time, 0);
        const weight = raw.weight !== undefined
            ? ensureNumber(raw.weight)
            : raw.load !== undefined
                ? ensureNumber(raw.load)
                : undefined;
        // Supporto nuovi campi: intensityPercent, rpe, weightLbs e ranges
        const intensityPercent = raw.intensityPercent !== undefined ? ensureNumber(raw.intensityPercent) : undefined;
        const intensityPercentMax = raw.intensityPercentMax !== undefined ? ensureNumber(raw.intensityPercentMax) : undefined;
 ...
 */
export function normalizeSetProgression(raw) {
    if (!raw || typeof raw !== 'object')
        return undefined;
    const progression = raw;
    const type = progression.type;
    if (type !== 'linear' && type !== 'percentage' && type !== 'rpe')
        return undefined;
 ...
 */
export function normalizeSetGroup(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const group = raw;
    ...
    const sets = group.sets && Array.isArray(group.sets)
 ...
 */
export function normalizeExercise(rawExercise, dayNumber, index) {
    ...
    const name = ensureString(raw.name ?? raw.exerciseName ?? raw.title ?? raw.exercise ?? `Esercizio ${index + 1}`, `Esercizio ${index + 1}`);
    ...
    const normalizeMuscleGroupLabel = (value) => {
        const result = typeof value === 'string' ? getMuscleGroupFromName(value) : null;
        return result ?? 'full-body';
    };
    const ensureMuscleGroups = (value) => {
        if (Array.isArray(value)) {
            const groups = value
                .map((entry) => normalizeMuscleGroupLabel(entry))
                .filter(Boolean);
    ...
    return {
        ...
        repRange: typeof rawReps === 'string' ? rawReps : repsNumber !== undefined ? `${repsNumber}` : '',
        ...
        catalogExerciseId: typeof raw.catalogExerciseId === 'string' && raw.catalogExerciseId.length > 0
