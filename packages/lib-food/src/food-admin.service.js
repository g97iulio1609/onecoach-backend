...
export class FoodAdminService {
    ...
    static async generateFoodsWithAgent(options) {
        const { count, description, existingFoods, categoryIds, userId, mergeExisting = false, onProgress, } = options;
        ...
        onProgress?.(generationStartProgress, `Iniziando generazione di ${count} alimenti in ${batches} batch...`);
        ...
        const allFoods = await processBatchesInParallel({
            ...
            processor: async (batch, _batchIndex) => {
                ...
                onProgress?.(generationStartProgress +
                    (batchIdx / batches) * (generationEndProgress - generationStartProgress), `Generando batch ${batchIdx + 1}/${batches} (${batchCount} alimenti)...`);
                ...
                const generatedFoods = (result.output?.foods || []);
                generatedCount += generatedFoods.length;
                onProgress?.(generationStartProgress +
                    ((batchIdx + 1) / batches) * (generationEndProgress - generationStartProgress), `Batch ${batchIdx + 1}/${batches} completato. Generati ${generatedCount}/${count} alimenti finora...`);
        ...
        const importRecords = allFoods
            ...
            if (f.barcode &&
                ...
                !['000000', '123456', '123456789', '111111', '999999', '0000000000000'].includes(f.barcode.trim())) {
            ...
            if (validatedCount % 5 === 0) {
                onProgress?.(validationStartProgress +
                    (validatedCount / allFoods.length) *
                        (validationEndProgress - validationStartProgress), `Validati ${validatedCount}/${allFoods.length} alimenti...`);
            }
            return record;
        })
            .filter((record) => {
            ...
            return (!!record.name && !!record.description && !!record.macrosPer100g && !!record.servingSize);
        });
        if (importRecords.length === 0) {
            throw new Error('No valid foods generated (missing required fields: name, description, macrosPer100g, or servingSize)');
        }
        onProgress?.(validationEndProgress, `${importRecords.length} alimenti validati. Iniziando import nel database...`);
        ...
        onProgress?.(importEndProgress, `Completato! ${importResult.created} creati, ${importResult.updated} aggiornati, ${importResult.skipped} saltati.`);
        return importResult;
    }
    /**
    ...
    static async import(records, options = {}) {
        ...
        const normalizedRecords = await Promise.all(records.map((record) => this.normalizeImportRecord(record)));
        const nameCache = new Map(); // name -> foodId
     ...
     */
    static async normalizeImportRecord(payload) {
