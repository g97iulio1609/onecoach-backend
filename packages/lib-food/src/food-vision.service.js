...
async function getVisionModelConfig(type) {
    ...
    const visionModels = metadata.visionModels || {};
    let model;
    if (type === 'label') {
 ...
 */
export async function updateVisionModelConfig(labelModel, segmentationModel) {
    const openRouterConfig = await AIProviderConfigService.getConfig('openrouter');
    if (!openRouterConfig) {
...
export class FoodVisionService {
     ...
     */
    static async extractLabelData(imageBase64, userId) {
        ...
        try {
            ...
            throw new Error(`Errore nell'estrazione dati: ${error instanceof Error ? error.message : 'Errore sconosciuto'}`);
        }
    }
    /**
     * Segmenta piatto identificando componenti e quantità
     */
    static async segmentDish(imageBase64, userId) {
        ...
        try {
            ...
            const filteredItems = segmentationResult.items.filter((item) => item.confidence ?? 0 >= 0.5);
            return {
                items: filteredItems,
            ...
            throw new Error(`Errore nella segmentazione: ${error instanceof Error ? error.message : 'Errore sconosciuto'}`);
        }
    }
}
