...
import { oneagendaKeys, tasksQueries, goalsQueries, } from '../queries/oneagenda.queries';
/**
...
export function useUpdateTaskStatus() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, status }) => tasksQueries.updateStatus(id, status),
        onMutate: async ({ id, status }) => {
            // Cancel any outgoing refetches
            await queryClient.cancelQueries({ queryKey: oneagendaKeys.tasks.lists() });
            // Snapshot previous value
            const previousTasks = queryClient.getQueryData(oneagendaKeys.tasks.list());
            // Optimistically update
            if (previousTasks) {
                queryClient.setQueryData(oneagendaKeys.tasks.list(), previousTasks.map((task) => (task.id === id ? { ...task, status } : task)));
            }
...
export function useDeleteTask() {
    ...
    return useMutation({
        ...
        onMutate: async (id) => {
            ...
            if (previousTasks) {
                queryClient.setQueryData(oneagendaKeys.tasks.list(), previousTasks.filter((task) => task.id !== id));
            }
            return { previousTasks };
        },
 ...
 */
export function useGoals(filters, enabled = true) {
    return useQuery({
...
export function useDeleteGoal() {
    ...
    return useMutation({
        ...
        onMutate: async (id) => {
            ...
            // Snapshot previous value
            const previousGoals = queryClient.getQueryData(oneagendaKeys.goals.list());
            // Optimistically remove
            if (previousGoals) {
                queryClient.setQueryData(oneagendaKeys.goals.list(), previousGoals.filter((goal) => goal.id !== id));
            }
...
export function useToggleHabit() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id) => import('../queries/oneagenda.queries').then((m) => m.habitsQueries.toggle(id)),
        onSuccess: () => {
