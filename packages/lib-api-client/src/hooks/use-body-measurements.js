...
import { bodyMeasurementsKeys, bodyMeasurementsQueries, } from '../queries/body-measurements.queries';
/**
 ...
 */
export function useBodyMeasurements(filters) {
    return useQuery({
 ...
 */
export function useBodyMeasurement(id, enabled = true) {
    return useQuery({
...
export function useCreateBodyMeasurement() {
    ...
    return useMutation({
        ...
        onMutate: async (newMeasurement) => {
            ...
            const previousMeasurements = queryClient.getQueryData(bodyMeasurementsKeys.list());
            ...
            if (previousMeasurements) {
                queryClient.setQueryData(bodyMeasurementsKeys.list(), [
        ...
        onSuccess: (data, _variables, context) => {
            ...
            if (context?.previousMeasurements) {
                queryClient.setQueryData(bodyMeasurementsKeys.list(), context.previousMeasurements.map((m) => m.id === context.tempId ? data : m));
            }
...
export function useUpdateBodyMeasurement() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, input }) => bodyMeasurementsQueries.update(id, input),
        onMutate: async ({ id, input }) => {
            // Cancel any outgoing refetches
            await queryClient.cancelQueries({ queryKey: bodyMeasurementsKeys.detail(id) });
            await queryClient.cancelQueries({ queryKey: bodyMeasurementsKeys.lists() });
            // Snapshot previous values
            const previousMeasurement = queryClient.getQueryData(bodyMeasurementsKeys.detail(id));
            const previousMeasurements = queryClient.getQueryData(bodyMeasurementsKeys.list());
            ...
            if (previousMeasurements) {
                queryClient.setQueryData(bodyMeasurementsKeys.list(), previousMeasurements.map((m) => m.id === id
                    ? {
                        ...m,
                        ...input,
                        updatedAt: new Date().toISOString(),
        ...
        onError: (_err, variables, context) => {
            ...
            if (context?.previousMeasurement) {
                queryClient.setQueryData(bodyMeasurementsKeys.detail(variables.id), context.previousMeasurement);
            }
            if (context?.previousMeasurements) {
                queryClient.setQueryData(bodyMeasurementsKeys.list(), context.previousMeasurements);
            }
...
export function useDeleteBodyMeasurement() {
    ...
    return useMutation({
        ...
        onMutate: async (id) => {
            ...
            if (previousMeasurements) {
                queryClient.setQueryData(bodyMeasurementsKeys.list(), previousMeasurements.filter((m) => m.id !== id));
            }
