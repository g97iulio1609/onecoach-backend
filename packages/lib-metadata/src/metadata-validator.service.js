 ...
 */
function findBestMatch(searchName, candidates) {
    ...
    const contains = candidates.find((c) => normalizeName(c.name).includes(normalized) || normalized.includes(normalizeName(c.name)));
    if (contains)
        return contains.id;
    return null;
}
 ...
 */
export async function validateEquipmentByName(names) {
    if (names.length === 0)
        return [];
 ...
 */
export async function validateMusclesByName(names) {
    if (names.length === 0)
        return [];
    // Check cache first - try individual lookups
    const cachedIds = [];
 ...
 */
export async function validateBodyPartsByName(names) {
    if (names.length === 0)
        return [];
    // Check cache first - try individual lookups
    const cachedIds = [];
 ...
 */
export async function validateExerciseTypeByName(name, sharedContext) {
    if (!name)
        return null;
    const normalized = normalizeName(name);
