...
const resolvedCache = new SimpleCache({
    max: 500,
    ttl: CACHE_TTL_MS,
});
/**
 ...
 */
function buildCacheKey(type, ids, locale) {
    return `${type}:${locale}:${ids.sort().join(',')}`;
}
/**
 * Resolve equipment IDs to names
 */
async function resolveEquipmentIds(ids, locale = DEFAULT_LOCALE) {
    if (ids.length === 0)
        return {};
    ...
    for (const eq of equipment) {
        const translation = eq.equipment_translations.find((t) => t.locale === locale) ||
 ...
 */
async function resolveMuscleIds(ids, locale = DEFAULT_LOCALE) {
    if (ids.length === 0)
        return {};
    ...
    for (const muscle of muscles) {
        const translation = muscle.muscle_translations.find((t) => t.locale === locale) ||
 ...
 */
async function resolveBodyPartIds(ids, locale = DEFAULT_LOCALE) {
    if (ids.length === 0)
        return {};
    const cacheKey = buildCacheKey('bodyParts', ids, locale);
    const cached = resolvedCache.get(cacheKey);
    if (cached?.bodyParts)
        return cached.bodyParts;
    ...
    for (const bp of bodyParts) {
        const translation = bp.body_part_translations.find((t) => t.locale === locale) ||
 ...
 */
async function resolveExerciseTypeIds(ids, locale = DEFAULT_LOCALE) {
    if (ids.length === 0)
        return {};
    const cacheKey = buildCacheKey('exerciseTypes', ids, locale);
    const cached = resolvedCache.get(cacheKey);
    if (cached?.exerciseTypes)
        return cached.exerciseTypes;
    ...
    for (const et of exerciseTypes) {
        const translation = et.exercise_type_translations.find((t) => t.locale === locale) ||
 ...
 */
export async function resolveMetadataBatch(ids, locale = DEFAULT_LOCALE) {
 ...
 */
export async function buildIdMapForAI(type, ids) {
    if (ids.length === 0)
        return {};
