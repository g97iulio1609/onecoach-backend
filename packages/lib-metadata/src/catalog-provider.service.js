...
export class CatalogProviderService {
     ...
     */
    static async getExerciseCatalog() {
        const now = Date.now();
        ...
        try {
            ...
            const catalog = exercises.map((exercise) => {
                ...
                if (exercise.exerciseTypeId?.includes('beginner'))
                    difficulty = 'BEGINNER';
                else if (exercise.exerciseTypeId?.includes('advanced'))
                    difficulty = 'ADVANCED';
     ...
     */
    static formatExerciseCatalogForPrompt(exercises) {
     ...
     */
    static async matchExerciseByName(exerciseName) {
        try {
     ...
     */
    static async matchExerciseBySearchTerms(searchQuery) {
        try {
