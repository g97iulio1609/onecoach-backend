...
export class PlanningServiceV2 {
    ...
    static async createPlan(params) {
        ...
        const mealsPerDay = structure?.mealsPerDay ||
            metadata?.mealsPerDay ||
        ...
        const shouldCreateSubSubTasks = agentType === 'nutrition' &&
            mealsPerDay &&
    ...
    static async getProgress(planId) {
        ...
        const totalSubTasks = plan.tasks.reduce((sum, task) => sum + task.subTasks.length, 0);
        const completedSubTasks = plan.tasks.reduce((sum, task) => sum + task.subTasks.filter((st) => st.status === 'completed').length, 0);
     ...
     */
    static async updateTaskStatus(planId, weekNumber, status, result, error) {
        try {
     ...
     * Atomic update con transaction
     */
    static async updateSubTaskStatus(planId, weekNumber, dayNumber, status, result, error) {
        try {
            await prisma.$transaction(async (tx) => {
                // Find task
                const task = await tx.planning_tasks.findFirst({
                ...
                if (!task)
                    throw new Error(`Task not found: week ${weekNumber}`);
                // Find subtask
                const subTask = task.subTasks.find((st) => st.dayNumber === dayNumber);
                if (!subTask)
                ...
                const updatedSubTasks = task.subTasks.map((st) => st.id === subTask.id ? { ...st, status: toPrismaStatus(status) } : st);
                ...
                else if (updatedSubTasks.some((st) => st.status === 'IN_PROGRESS')) {
                    await tx.planning_tasks.update({
                        where: { id: task.id },
                        data: {
                            status: 'IN_PROGRESS',
     ...
     */
    static async getNextTask(planId) {
        const plan = await this.getPlan(planId);
     ...
     */
    static async aggregateResults(planId) {
        const plan = await this.getPlan(planId);
     ...
     */
    static async removePlan(planId) {
        try {
     ...
     */
    static async resumePlan(planId) {
        try {
            await prisma.planning_plans.update({
                where: { id: planId },
                data: {
     ...
     */
    static async setPlanMetadata(planId, patch) {
        try {
            const plan = await prisma.planning_plans.findUnique({
                where: { id: planId },
                select: { metadata: true },
            });
            const current = plan?.metadata || {};
    ...
    static async getUserPlans(userId) {
        ...
        return dbPlans.map((p) => this.mapDbPlanToInterface(p));
    }
    // ============================================================================
    // PRIVATE HELPERS
    // ============================================================================
     ...
     */
    static async updatePlanStatusInTransaction(tx, planId) {
     ...
     */
    static mapDbPlanToInterface(dbPlan) {
        return {
            id: dbPlan.id,
            agentType: dbPlan.agentType.toLowerCase(),
            ...
            metadata: dbPlan.metadata || undefined,
            tasks: dbPlan.tasks.map((task) => ({
                id: task.id,
     ...
     */
    static aggregateWorkoutResults(plan) {
        const weeks = plan.tasks
            ...
            if (!task.result) {
                ...
                const days = task.subTasks
                    ...
                    .map((st) => {
                    if (!st.result) {
                        throw new Error(`SubTask giorno ${st.dayNumber} settimana ${task.weekNumber} non ha risultato`);
                    }
                    return st.result;
                });
                return {
     ...
     */
    static aggregateNutritionResults(plan) {
        const weeks = plan.tasks
            .sort((a, b) => a.weekNumber - b.weekNumber)
            .map((task) => {
            if (!task.result) {
    ...
    static async createSubSubTasksInTransaction(tx, subTaskId, mealsPerDay) {
        const { inferMealType, getDefaultMealName } = await import('@onecoach/lib-ai-agents/schemas/meal-schemas');
     ...
     * Atomic update con transaction
     */
    static async updateSubSubTaskStatus(planId, weekNumber, dayNumber, mealNumber, status, result, error) {
        try {
            await prisma.$transaction(async (tx) => {
                // Find task
                const task = await tx.planning_tasks.findFirst({
                ...
                if (!subTask)
                    throw new Error(`SubTask not found: day ${dayNumber}`);
                // Find sub-sub-task
                const subSubTask = subTask.subSubTasks.find((sst) => sst.mealNumber === mealNumber);
                if (!subSubTask) {
     ...
     */
    static async getSubSubTasks(planId, weekNumber, dayNumber) {
        const plan = await this.getPlan(planId);
        const task = plan.tasks.find((t) => t.weekNumber === weekNumber);
        if (!task) {
            throw new Error(`Task not found: week ${weekNumber}`);
        }
        const subTask = task.subTasks.find((st) => st.dayNumber === dayNumber);
