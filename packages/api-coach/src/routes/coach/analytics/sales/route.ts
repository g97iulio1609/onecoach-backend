/**
 * Coach Analytics Sales API
 * GET /api/coach/analytics/sales - Get sales trends for coach
 */

import { NextRequest, NextResponse } from 'next/server';
import { auth, roleSatisfies } from '@onecoach/lib-core/auth';
import { prisma } from '@onecoach/lib-core';
import { getSalesTrends, type Period } from '@onecoach/lib-analytics';
import { logError, mapErrorToApiResponse } from '@onecoach/lib-shared';

export const dynamic = 'force-dynamic';

/**
 * GET /api/coach/analytics/sales
 * Get sales trends for coach
 */
export async function GET(_request: NextRequest) {
  try {
    ...
    const trends = await getSalesTrends(session.user.id, period);
