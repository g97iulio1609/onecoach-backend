...
export class FoodAutoCreationService {
     ...
     */
    static async batchProcessFoods(foods) {
        const result = {
            resolved: new Map(),
            created: 0,
            matched: 0,
            ...
            if (settled.status === 'fulfilled') {
                ...
                if (resolution) {
                    ...
                    if (resolution.status === 'created')
                        ...
                        result.matched++;
                    else if (resolution.status === 'existing')
                        result.existing++;
                }
                if (error) {
     ...
     */
    static async matchOrCreateFood(food) {
        const normalizedName = normalizeFoodName(food.name);
    ...
    static async createFoodInDatabase(food) {
        ...
        const proteinPct = Math.min(100, Math.max(0, ((food.macrosPer100g.protein || 0) * 4 * 100) / totalKcal));
        const carbPct = Math.min(100, Math.max(0, ((food.macrosPer100g.carbs || 0) * 4 * 100) / totalKcal));
        const fatPct = Math.min(100, Math.max(0, ((food.macrosPer100g.fats || 0) * 9 * 100) / totalKcal));
        // Calcola mainMacro
        const mainMacro = calculateMainMacro(food.macrosPer100g);
        // Gestione brand
        let brandId;
     ...
     */
    static ensureValidDescription(description, foodName) {
     ...
     */
    static findBestMatch(targetName, targetMacros, searchResults) {
        let bestMatch = null;
        let bestScore = 0;
    ...
    static levenshteinDistance(str1, str2) {
        ...
        for (let i = 1; i <= len1; i++) {
            for (let j = 1; j <= len2; j++) {
                ...
                if (row && prevRow) {
                    row[j] = Math.min((prevRow[j] ?? Infinity) + 1, (row[j - 1] ?? Infinity) + 1, (prevRow[j - 1] ?? Infinity) + cost);
                }
            }
        }
     ...
     */
    static macroSimilarity(macros1, macros2) {
     ...
     */
    static async processNutritionPlan(plan) {
        // Estrai tutti gli alimenti dal piano
        const foodsToProcess = [];
        const foodIdToNormalizedName = new Map();
     ...
     */
    static async validatePlanFoods(plan) {
        const missingFoods = [];
        for (const week of plan.weeks) {
            for (const day of week.days) {
                for (const meal of day.meals) {
     ...
     */
    static convertAIFoodsToProcess(aiFoods) {
        const result = [];
