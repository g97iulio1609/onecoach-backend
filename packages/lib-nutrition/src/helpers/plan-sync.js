 ...
 */
import { calculateMacros, aggregateMealMacros, normalizeMacros, } from '@onecoach/lib-shared/utils/macro-calculations';
/**
 * Sincronizza piano per il salvataggio
 * Normalizza tutti i valori numerici, calcola macro totali, pulisce array
 */
export function syncPlanForSave(planToSync) {
    const syncedWeeks = (planToSync.weeks || []).map((week) => {
        const syncedDays = (week.days || []).map((day) => {
                ...
                const foods = meal.foods.map((food) => ({
                    ...
                    macros: food.macros
                        ? normalizeMacros({
                            ...
                            fiber: food.macros?.fiber !== undefined && Number.isFinite(food.macros.fiber)
    ...
    return {
        ...planToSync,
        weeks: syncedWeeks,
        restrictions: (planToSync.restrictions ?? []).map((r) => r.trim()).filter(Boolean),
        preferences: (planToSync.preferences ?? []).map((p) => p.trim()).filter(Boolean),
    };
}
