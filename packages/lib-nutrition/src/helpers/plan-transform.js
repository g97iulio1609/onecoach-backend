...
import { NutritionWeekSchema, NutritionDaySchema, MealSchema, FoodSchema, PersonalizedPlanSchema, AdaptationsSchema, NutritionPlanBaseSchema, } from '@onecoach/schemas';
import { generateId } from '@onecoach/lib-shared/utils/id-generator';
import { calculateMacros, aggregateMealMacros, normalizeMacros, normalizeMacroValue, } from '@onecoach/lib-shared/utils/macro-calculations';
import { parseCompleteMacrosSafe, isNutritionWeek, isNutritionDay, isMeal, isFood, isPersonalizedPlan, isAdaptations, isMetadata, } from './type-guards';
...
function normalizePersonalizedPlanValue(value) {
    if (!value)
        return undefined;
    if (isPersonalizedPlan(value))
        return value;
    const parsed = PersonalizedPlanSchema.safeParse(value);
    if (!parsed.success)
        return undefined;
    const maybe = parsed.data;
    const macroSplit = maybe.macroSplit;
    const recommendations = Array.isArray(maybe.recommendations)
        ? maybe.recommendations.filter((r) => typeof r === 'string' && r.length > 0)
        : [];
    const customizations = Array.isArray(maybe.customizations)
        ? maybe.customizations.filter((r) => typeof r === 'string' && r.length > 0)
        : [];
    const personalNotes = Array.isArray(maybe.personalNotes)
        ? maybe.personalNotes.filter((r) => typeof r === 'string' && r.length > 0)
        : [];
    const mergedRecommendations = [...recommendations, ...customizations, ...personalNotes];
    if (typeof maybe.tdee === 'number' &&
        ...
        return {
            ...
            recommendations: mergedRecommendations.length > 0 ? mergedRecommendations : ['Personalized plan'],
        };
    }
    ...
    return {
        ...
        macroSplit: {
            protein: typeof macroSplit?.protein === 'number' ? macroSplit.protein : 0,
            carbs: typeof macroSplit?.carbs === 'number' ? macroSplit.carbs : 0,
            fats: typeof macroSplit?.fats === 'number' ? macroSplit.fats : 0,
        },
        recommendations: mergedRecommendations.length > 0 ? mergedRecommendations : ['Personalized plan'],
    };
}
function normalizeAdaptationsValue(value) {
    if (!value) {
        return undefined;
    }
...
// ============================================
function isValidMacros(value) {
    if (!value || typeof value !== 'object')
        return false;
    const m = value;
    // Accept both numbers and strings (will be normalized in parseMacros)
    return ((typeof m.calories === 'number' || typeof m.calories === 'string') &&
...
function isValidGoalsArray(value) {
...
function parseUserProfile(value) {
    ...
    if (typeof value === 'object' && value !== null) {
        const v = value;
        const asNumber = (val, fallback) => typeof val === 'number' ? val : Number(val) || fallback;
        const sex = v.sex === 'male' || v.sex === 'female'
            ? v.sex
            : (String(v.sex || '').toLowerCase() === 'female' ? 'female' : 'male');
        return {
 ...
 */
export function parseNutritionStatus(value) {
    if (typeof value === 'string') {
        const upper = value.toUpperCase();
        if (Object.values(NutritionStatus).includes(upper)) {
            return upper;
        }
    }
    if (value && Object.values(NutritionStatus).includes(value)) {
        return value;
    }
...
function normalizeWeek(week, index) {
    ...
    const normalizedDays = days.map((d, i) => normalizeDay(d, i));
    ...
    if (!rawWeek.weeklyAverageMacros && normalizedDays.length > 0) {
        const totals = normalizedDays.reduce((acc, day) => ({
...
function normalizeDay(day, index) {
    ...
    const normalizedMeals = meals.map((m, i) => normalizeMeal(m, rawDay.dayNumber ?? index + 1, i));
...
function normalizeMeal(meal, dayNumber, index) {
    ...
    const normalizedFoods = foods.map((f, i) => normalizeFood(f, generateId(`food_${dayNumber}_${index}_${i}`)));
 ...
 */
function normalizeFood(food, fallbackId) {
    ...
    const foodItemId = rawFood.foodItemId ||
        rawFood.itemId ||
    ...
    const name = rawFood.name || rawFood.foodName || rawFood.food_name || rawFood.displayName || 'Unknown Food';
 ...
 */
export function preparePlanForPersistence(plan) {
    return {
        name: plan.name,
        description: plan.description,
        goals: plan.goals,
            ...
            days: week.days.map((day) => ({
                ...
                meals: day.meals.map((meal) => ({
                    ...
                    foods: meal.foods.map((food) => {
                        ...
                        if (!food.foodItemId) {
                            console.warn('[SCHEMA_VERIFY] Food without foodItemId in preparePlanForPersistence:', {
                                foodId: food.id,
                                foodName: food.name,
                        ...
                        return {
                            ...
                            quantity: typeof food.quantity === 'number' ? food.quantity : Number(food.quantity) || 0,
                            unit: food.unit,
                            notes: food.notes,
                            done: food.done,
...
// BUILDERS
// ============================================
export function createEmptyDay(dayNumber) {
    return {
...
export function createEmptyPlan(userId) {
    return {
 ...
 */
export function normalizeAgentPayload(payload, base) {
    ...
    if (!normalizedGoals || !Array.isArray(normalizedGoals) || normalizedGoals.length === 0) {
        if (base?.goals && Array.isArray(base.goals) && base.goals.length > 0) {
            ...
            throw new Error('Invalid agent payload: goals must be a non-empty array of strings (validated by Zod schema)');
        }
    }
    if (!base?.userId) {
        throw new Error('userId is required to normalize agent payload');
    }
    const normalizedUserProfile = parseUserProfile(raw.userProfile ?? base?.userProfile);
    ...
    if (!validationResult.success) {
        ...
        throw new Error(`Invalid agent payload structure: ${validationResult.error.issues.map((i) => i.message).join(', ')}`);
    }
    return {
