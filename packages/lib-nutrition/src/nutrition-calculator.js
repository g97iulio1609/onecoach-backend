 ...
 */
export const PROTEIN_PER_KG_BY_GOAL = {
 ...
 */
export const DEFAULT_MACRO_RATIOS_BY_DIET = {
 ...
 */
export function calculateCalorieNeeds(metrics) {
 ...
 */
export function calculateMacrosFromRatios(targetCalories, ratios) {
 ...
 */
export function calculateMacrosByDietType(targetCalories, dietType, weightKg, goal) {
 ...
 */
export function suggestTargetCalories(tdee, goal) {
    switch (goal) {
        case 'weight_loss':
 ...
 */
export function validateNutritionTargets(targets, weightKg) {
    const errors = [];
    const warnings = [];
    // 1. Valida calorie totali
    if (targets.targetCalories < SAFETY_RANGES.minCalories) {
        errors.push(`Calorie ${targets.targetCalories} sotto il minimo sicuro (${SAFETY_RANGES.minCalories})`);
    }
    if (targets.targetCalories > SAFETY_RANGES.maxCalories) {
        errors.push(`Calorie ${targets.targetCalories} sopra il massimo sicuro (${SAFETY_RANGES.maxCalories})`);
    }
    // 2. Valida deficit/surplus
    if (targets.deficit && targets.deficit > SAFETY_RANGES.maxDeficit) {
        errors.push(`Deficit ${targets.deficit} kcal troppo aggressivo (max ${SAFETY_RANGES.maxDeficit})`);
    }
    ...
    if (fatRatio < SAFETY_RANGES.fat.min) {
        errors.push(`Grassi ${(fatRatio * 100).toFixed(0)}% sotto il minimo (${SAFETY_RANGES.fat.min * 100}%)`);
    }
    if (fatRatio > SAFETY_RANGES.fat.max) {
        warnings.push(`Grassi ${(fatRatio * 100).toFixed(0)}% sopra il massimo raccomandato (${SAFETY_RANGES.fat.max * 100}%)`);
    }
    // 5. Valida somma macro = calorie
    const calculatedCalories = targets.macros.protein * KCAL_PER_GRAM.protein +
    ...
    if (caloriesDiff > 50) {
        warnings.push(`Macro non coerenti: ${calculatedCalories} kcal vs target ${targets.targetCalories} kcal`);
    }
...
export function validateMacroRatios(ratios) {
    ...
    if (ratios.fatRatio < SAFETY_RANGES.fat.min) {
        errors.push(`Fat ratio ${(ratios.fatRatio * 100).toFixed(0)}% < ${SAFETY_RANGES.fat.min * 100}%`);
    }
    if (ratios.fatRatio > SAFETY_RANGES.fat.max) {
        warnings.push(`Fat ratio ${(ratios.fatRatio * 100).toFixed(0)}% > ${SAFETY_RANGES.fat.max * 100}%`);
    }
 ...
 */
export function calculateCompleteNutritionTargets(metrics, goal, dietType = 'omnivore') {
...
export function macrosToCalories(macros) {
    return (macros.protein * KCAL_PER_GRAM.protein +
 ...
 */
export function formatNutritionTargets(targets) {
