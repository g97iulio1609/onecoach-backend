...
import { normalizeNutritionPlan, preparePlanForPersistence, normalizeAgentPayload, } from './helpers/plan-transform';
...
export class ModificationService {
     ...
     */
    static async modifyNutritionDay(params) {
        ...
        const { getNutritionPlanDay, getNutritionPlanTotalDays } = await import('@onecoach/lib-shared/utils/nutrition-plan-helpers');
        ...
        const updatedWeeks = normalizedPlanToUpdate.weeks.map((week) => {
            if (week.weekNumber === weekNum) {
                return {
                    ...week,
                    days: week.days?.map((d) => (d.dayNumber === dayNumber ? modifiedDay : d)) ||
                        [],
                };
            }
            return week;
        ...
        await prisma.$transaction(async (tx) => {
            await tx.nutrition_plan_versions.create({
                data: {
                    ...
                    targetMacros: planToUpdate.targetMacros,
                    weeks: planToUpdate.weeks,
                    restrictions: planToUpdate.restrictions,
                    preferences: planToUpdate.preferences,
                    status: planToUpdate.status,
                    metadata: planToUpdate.metadata ?? Prisma.JsonNull,
                    createdBy: userId,
                },
            });
     ...
     */
    static async modifyWorkoutWeek(params) {
        ...
        if (weekNumber < 1 || weekNumber > normalizedProgramToUpdate.weeks.length) {
            throw new Error(`Invalid week number: ${weekNumber}. Program has ${normalizedProgramToUpdate.weeks.length} week(s)`);
        }
        ...
        const { calculateWeightsInProgram } = await import('@onecoach/lib-workout/workout-weight-calculator.service');
        const programWithSyncedWeights = await calculateWeightsInProgram(userId, programWithModifiedWeek);
        ...
        await prisma.$transaction(async (tx) => {
            await tx.workout_program_versions.create({
                data: {
                    ...
                    weeks: programToUpdate.weeks,
                    metadata: programToUpdate.metadata ??
                        Prisma.JsonNull,
                    createdBy: userId,
                },
            });
