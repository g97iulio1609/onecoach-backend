 ...
 */
function formatDateForGroup(date, groupBy) {
    if (groupBy === 'day') {
 ...
 */
export async function getSalesTrends(userId, period = '30d') {
 ...
 */
export async function getRevenueTrends(userId, period = '30d') {
    const { startDate, endDate } = getDateRange(period);
    const groupBy = groupByDate(period);
    // Get all completed purchases for coach's plans
    const purchases = await prisma.plan_purchases.findMany({
 ...
 */
export async function getRatingTrends(userId, period = '30d') {
    const { startDate, endDate } = getDateRange(period);
    const groupBy = groupByDate(period);
    ...
    while (currentDate <= end) {
        ...
        const avgRating = group && group.ratings.length > 0
 ...
 */
export async function getTopPlans(userId, limit = 5) {
    ...
    const plansWithStats = plans.map((plan) => {
        ...
        const revenue = plan.plan_purchases.reduce((sum, p) => sum + Number(p.coachCommission), 0);
