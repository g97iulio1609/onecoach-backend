...
import { AIFrameworkConfigService, FrameworkFeature, } from '@onecoach/lib-ai';
// ==================== CONFIG ====================
const VISION_CONFIG = {
...
async function getVisionModelConfig(fileType) {
    ...
    if (!model)
        throw new Error(`Modello AI per tipo "${fileType}" non configurato.`);
    ...
    if (creditCost === undefined)
        throw new Error(`Costo crediti per "${fileType}" non configurato.`);
    return {
        model,
        fallbackModel,
        apiKey,
...
export async function parseWithVisionAI(params) {
    const { contentBase64, mimeType, prompt, schema, userId, fileType, creditCost: overrideCreditCost, modelId: overrideModelId, apiKey: overrideApiKey, onProgress, } = params;
    // Load config
    ...
    if (!hasCredits)
        throw new Error(`Crediti insufficienti. Richiesti: ${creditCost}`);
...
async function callVisionAI(params) {
...
async function callTextAI(params) {
    ...
    try {
        ...
        if (textContent.startsWith('PK'))
            return callVisionAI(params); // Binary XLSX fallback
    }
    catch {
