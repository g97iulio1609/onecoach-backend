...
const AgendaModificationChangesSchema = z.object({
  ...
  // Status/Priority
  status: z.enum(['TODO', 'IN_PROGRESS', 'COMPLETED', 'ARCHIVED', 'ON_HOLD', 'ACTIVE']).optional()
    .describe('New status. Task: TODO/IN_PROGRESS/COMPLETED. Project: ACTIVE/COMPLETED/ARCHIVED/ON_HOLD'),
...
const agendaApplyModificationParams = z.object({
  ...
  execute: async (args, _context: McpContext) => {
    ...
    switch (action) {
      case 'update_project': {
        ...
        if (changes?.status) updateData.status = changes.status as any; // Cast needed as enum might differ slightly or be safe
      ...
      case 'update_task': {
        ...
        if (changes?.title) updateData.title = changes.title;
        if (changes?.description) updateData.description = changes.description;
        if (changes?.status) updateData.status = changes.status as any;
        if (changes?.priority) updateData.priority = changes.priority as any;
