...
export async function resolveExerciseByName(name: string): Promise<ResolvedExercise | null> {
  ...
  const result: ResolvedExercise = {
    ...
    muscleGroups: ex.exercise_muscles.map((m: { muscles?: { name?: string } }) => m.muscles?.name).filter(Boolean) as string[],
    equipment: ex.exercise_equipments.map((e: { equipments?: { name?: string } }) => e.equipments?.name).filter(Boolean) as string[],
  };
 ...
 */
export async function searchExercisesCatalog(query: string, limit = 10): Promise<ResolvedExercise[]> {
  const translations = await prisma.exercise_translations.findMany({
    where: {
