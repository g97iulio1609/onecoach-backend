...
import { AffiliateRewardStatus, AffiliateRewardType, Prisma, ReferralAttributionStatus, } from '@prisma/client';
...
export class AffiliateService {
     ...
     */
    static async ensureUserReferralCode(userId, programId) {
    ...
    static async handleInvoicePaid(params) {
        ...
        const pendingUntil = new Date(params.occurredAt.getTime() + program.rewardPendingDays * DAY_IN_MS);
        for (const attribution of attributions) {
            if (attribution.status === ReferralAttributionStatus.CANCELLED &&
                attribution.graceEndAt &&
                params.occurredAt > attribution.graceEndAt) {
                continue;
            }
            const levelConfig = program.affiliate_program_levels.find((level) => level.level === attribution.level);
    ...
    static async handleSubscriptionCancellation(params) {
        ...
        await Promise.all(attributions.map((attribution) => prisma.referral_attributions.update({
            where: { id: attribution.id },
            data: {
                status: ReferralAttributionStatus.CANCELLED,
                cancelledAt: params.occurredAt,
                graceEndAt: new Date(params.occurredAt.getTime() +
     ...
     */
    static async releasePendingRewards(referenceDate = new Date()) {
    ...
    static async applyReferralCode(params) {
        ...
        const createdAttributionIds = [];
        await prisma.$transaction(async (tx) => {
            let parentAttributionId;
            for (const chainItem of referralChain) {
                ...
                const levelConfig = program.affiliate_program_levels.find((level) => level.level === chainItem.level);
                const creditReward = levelConfig?.creditReward ?? (chainItem.level === 1 ? program.registrationCredit : 0);
    ...
    static async buildReferralChain(params) {
        const chain = [];
        let currentLevel = 1;
        let currentCodeId = params.startingCode.id;
        let currentUserId = params.startingCode.userId;
        ...
        const parentAttributionByReferredUser = new Map();
     ...
     */
    static async getOrCreateReferralCode(userId, programId) {
    ...
    static async getAffiliateStats(userId) {
        const [totalRewards, pendingRewards, clearedRewards, cancelledRewards, totalAttributions] = await Promise.all([
     ...
     */
    static async approvePayout(payoutId, adminUserId, notes) {
     ...
     */
    static async rejectPayout(payoutId, adminUserId, reason) {
        const reward = await prisma.affiliate_rewards.findUnique({
            where: { id: payoutId },
        });
        if (!reward) {
     ...
     */
    static async markPayoutPaid(payoutId, adminUserId) {
        const reward = await prisma.affiliate_rewards.findUnique({
            where: { id: payoutId },
        });
        if (!reward) {
