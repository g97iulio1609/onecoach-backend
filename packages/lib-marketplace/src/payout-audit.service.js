...
export class PayoutAuditService {
    ...
    static async createAuditLog(params) {
        const { userId, rewardIds, action, amount, currencyCode, performedBy, notes, metadata } = params;
     ...
     */
    static async getAuditLogsByUser(userId, limit = 50) {
     ...
     */
    static async getAuditLogsByPerformer(performedBy, limit = 50) {
        return prisma.payout_audit_log.findMany({
            where: { performedBy },
            orderBy: { createdAt: 'desc' },
            take: limit,
