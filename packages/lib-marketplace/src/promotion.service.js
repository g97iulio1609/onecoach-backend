...
export class PromotionService {
    ...
    static async createPromotion(params) {
        const { code, type, stripeCouponId, discountType, discountValue, bonusCredits, maxUses, maxUsesPerUser = 1, validFrom, validUntil, description, createdBy, } = params;
        // Validazione input
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (type === 'STRIPE_COUPON' && !stripeCouponId) {
                try {
                    ...
                    throw new Error(`Errore creazione coupon Stripe: ${error instanceof Error ? error.message : 'Unknown'}`);
                }
            }
     ...
     */
    static async validatePromotionCode(code, userId) {
        if (!code || code.trim().length === 0) {
     ...
     */
    static async getUserPromotionUses(promotionId, userId) {
     ...
     */
    static async applyBonusCredits(promotionId, userId, appliedBy) {
        const promotion = await prisma.promotions.findUnique({
     ...
     */
    static async applyPromotionToCheckout(code, userId) {
