...
export async function requireAdmin() {
    ...
    if (!isAdminRole(user.role)) {
        return NextResponse.json({ error: 'Accesso negato - Richiesti privilegi admin' }, { status: 403 });
    }
    return user;
}
/**
...
export async function requireSuperAdmin() {
    ...
    if (!isSuperAdminRole(user.role)) {
        return NextResponse.json({ error: 'Accesso negato - Richiesti privilegi super admin' }, { status: 403 });
    }
    return user;
}
/**
 ...
 */
export async function requireResourceAccess(resourceUserId) {
    const user = await getCurrentUser();
    if (!user) {
        return NextResponse.json({ error: 'Non autenticato' }, { status: 401 });
    }
