...
export function useLiveSessionSync(options) {
    const { sessionId, programId, totalSets, autoInitialize = true } = options;
    const initializedRef = useRef(false);
    const { liveSession, initLiveSessionContext, updateLiveSessionProgress, setCurrentExercise: storeSetCurrentExercise, recordCompletedSet: storeRecordSet, updateRestTimer, setLiveSessionStatus, clearLiveSession, } = useCopilotActiveContextStore();
    ...
    // Progress tracking
    const setProgress = useCallback((exerciseIndex, setIndex, completedSets) => {
        updateLiveSessionProgress(exerciseIndex, setIndex, completedSets);
    }, [updateLiveSessionProgress]);
    // Set current exercise
    const setCurrentExercise = useCallback((exerciseName, exerciseIndex) => {
    ...
    const recordSet = useCallback((setData) => {
        storeRecordSet({
    ...
    // Update rest timer
    const updateTimer = useCallback((running, secondsRemaining) => {
