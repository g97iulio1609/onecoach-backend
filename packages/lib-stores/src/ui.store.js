...
const getInitialState = () => {
    ...
    const initialActualTheme = 'light'; // Will be updated on mount
    return {
        theme: initialTheme,
    ...
    setTheme: (theme) => {
        ...
        if (typeof window !== 'undefined' && typeof document !== 'undefined') {
            ...
            if (themeColorMetaTag) {
                themeColorMetaTag.setAttribute('content', newActualTheme === 'dark' ? '#0f172a' : '#ffffff');
            }
        }
    },
...
export const useSidebar = () => {
    ...
    const subscribe = useCallback((callback) => useUIStore.subscribe(callback), []);
...
export function useSystemThemeSync() {
    ...
    useEffect(() => {
        if (typeof window === 'undefined')
            return;
        try {
            const storedTheme = localStorage.getItem('onecoach-theme');
 ...
 * ```
 */
export function useSystemThemeSyncNative(nativeColorScheme) {
    const setActualTheme = useUIStore((state) => state.setActualTheme);
    const theme = useUIStore((state) => state.theme);
    useEffect(() => {
